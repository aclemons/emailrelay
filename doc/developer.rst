...
Also at that time multi-threading support in C++ libraries was poor, so up
until version 2.0 there has been no reliance on any multi-threading libraries.
...
If only small parts of the implementation are o/s-specific then there might be
...
heap and the objects delete themselves when they receive certain events from the
event loop. In the *GNet* library it is the *ServerPeer* and *HeapClient*
...
is used in a future/promise pattern to wrap up *getaddrinfo()* and *waitfor()*
...
(\ *--as-install*\ ). When it runs it checks whether it has a payload of files
to install. If it has then it runs as an installer; if it does not then it runs
as a configuration helper. Refer to the comments in *src/gui/guimain.cpp* for
more details.
...
the *emailrelay.conf* file (or *emailrelay-start.bat* on Windows) and/or the
*emailrelay.auth* secrets file.

When run in *--as-install* mode the GUI expects to unpack all the E-MailRelay
files from a *payload* into target directories. Previously the payload was a
single archive file appended to the executable, but it is now simple directory
tree. On Mac OS X this is stored as a resource within the application bundle.
