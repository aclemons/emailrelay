...
bool G::BatchFile::ignorable( const std::string & line )
{
	return 
		line.empty() || 
		G::Str::lower(line+" ").find("@echo ") == 0U ||
		G::Str::lower(line+" ").find("rem ") == 0U ;
}

bool G::BatchFile::relevant( const std::string & line )
{
	return ! ignorable( line ) ;
}
...
std::string G::BatchFile::readFrom( std::istream & stream , const std::string & stream_name , bool strict )
{
	...
	{
		std::string s = G::Str::readLineFrom( stream ) ;
		if( !stream ) break ;
	...
	if( !line.empty() )
	{
		...
		size_type start_pos = G::Str::lower(line).find(start) ;
...
void G::BatchFile::write( const G::Path & path , const StringArray & args , const std::string & name_in )
{
	...
	if( name.empty() )
	{
		name = args.at(0U) ;
		dequote( name ) ;
		G::Path pname( name ) ; pname.removeExtension() ;
		name = pname.basename() ;
	}
