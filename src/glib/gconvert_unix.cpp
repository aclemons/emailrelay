...
public:
	ConvertImp( const ConvertImp & ) = delete ;
	void operator=( const ConvertImp & ) = delete ;

private:
	void reset() ;
	static std::wstring to_wstring( const std::vector<char> & ) ;
	static void from_wstring( std::vector<char> & , const std::wstring & ) ;
	static std::size_t call( std::size_t (*fn)(iconv_t,const char**,size_t*,char**,size_t*) ,
		iconv_t m , const char ** inbuf , std::size_t * inbytesleft ,
		char ** outbuf , std::size_t * outbytesleft ) ;
	static std::size_t call( std::size_t (*fn)(iconv_t,char**,size_t*,char**,size_t*) ,
		iconv_t m , const char ** inbuf , std::size_t * inbytesleft ,
		char ** outbuf , std::size_t * outbytesleft ) ;
...
std::size_t G::ConvertImp::call( std::size_t (*fn)(iconv_t,const char**,size_t*,char**,size_t*) ,
	iconv_t m , const char ** inbuf , std::size_t * inbytesleft ,
	char ** outbuf , std::size_t * outbytesleft )
{
...
std::size_t G::ConvertImp::call( std::size_t (*fn)(iconv_t,char**,size_t*,char**,size_t*) ,
	iconv_t m , const char ** inbuf , std::size_t * inbytesleft ,
	char ** outbuf , std::size_t * outbytesleft )
{
	char * p = const_cast<char*>(*inbuf) ;
	std::size_t rc = (*fn)( m , &p , inbytesleft , outbuf , outbytesleft ) ;
	*inbuf = p ;
	return rc ;
}
...
std::wstring G::ConvertImp::widen( const std::string & s , const std::string & context )
{
	...
	std::size_t in_n_start = s.size() ;
	const char * in_p = in_p_start ;
	std::size_t in_n = in_n_start ;

	// out-buffer
	std::vector<char> out_buffer( 10U + s.size()*4U ) ; //kiss
	char * out_p_start = &out_buffer[0] ;
	std::size_t out_n_start = out_buffer.size() ;
	char * out_p = out_p_start ;
	std::size_t out_n = out_n_start ;

	// iconv()
	//G_DEBUG( "G::ConvertImp::widen: in...\n" << hexdump<16>(in_p_start,in_p_end) ) ;
	std::size_t rc = call( ::iconv , m , &in_p , &in_n , &out_p , &out_n ) ;
	const auto e = static_cast<std::size_t>(ssize_t(-1)) ;
...
std::string G::ConvertImp::narrow( const std::wstring & s , const std::string & context )
{
	...
	std::size_t in_n_start = in_buffer.size() ;
	const char * in_p = in_p_start ;
	std::size_t in_n = in_n_start ;
...
std::wstring G::ConvertImp::to_wstring( const std::vector<char> & buffer )
{
	...
	const auto * p = reinterpret_cast<const unsigned char*>(&buffer[0]) ;
	...
	for( ; (p+1) < end ; p += 2 )
	{
		auto w = static_cast<wchar_t>( ( static_cast<unsigned int>(p[hi]) << 8 ) | p[lo] ) ;
		result.append( 1U , w ) ;
	}
	return result ;
}
