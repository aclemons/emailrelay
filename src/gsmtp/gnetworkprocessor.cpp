...
void GSmtp::NetworkProcessor::start( const std::string & path )
{
	...
	{
		m_client.reset( new ScannerClient(m_resolver_info,m_connection_timeout,m_response_timeout) ) ;
	}
	m_text.erase() ;
	m_client->startScanning( G::Path(path) ) ; // (no need to wait for connection)
}
