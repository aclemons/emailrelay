...
void GSmtp::ServerSend::sendUnrecognised( const std::string & line_in )
{
	std::string line = line_in.substr( 0U , 80U ) ;
	if( line.size() >= 80U ) line.append( " ..." ) ;
	send( "500 command unrecognized: \"" + line + std::string(1U,'\"') ) ;
}
...
void GSmtp::ServerSend::sendEhloReply( const Advertise & advertise )
{
	...
	if( advertise.chunking )
		ss << "250-CHUNKING" << crlf << "250-BINARYMIME" << crlf ; // RFC-3030
