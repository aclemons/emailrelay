...
{
	G::StringTokenT<G::string_view> token( bdat_line , "\t "_sv ) ;
	std::size_t size = 0U ;
	bool ok = false ;
	if( token && ++token )
	{
...
std::pair<bool,bool> GSmtp::ServerParser::parseBdatLast( G::string_view bdat_line )
{
	...
	if( token && ++token )
	{
		ok = true ;
		if( ++token )
			ok = last = G::Str::imatch("LAST",4U,token()) ;
	}
	return {last,ok} ;
}
...
{
	G::StringTokenT<G::string_view> t( line , " \t"_sv ) ;
	if( !G::Str::imatch("MAIL",4U,t()) || G::Str::ifind(t.next()(),"FROM:"_sv) != 0U )
...
{
	G::StringTokenT<G::string_view> t( line , " \t"_sv ) ;
	if( !G::Str::imatch("RCPT",4U,t()) || G::Str::ifind(t.next()(),"TO:"_sv) != 0U )
...
std::string GSmtp::ServerParser::parseMailStringValue( G::string_view line , G::string_view key_eq , AddressCommand & out , Conversion conversion )
{
	...
	if( out.error.empty() && out.tailpos != std::string::npos && out.tailpos < line.size() )
	{
		G::StringTokenT<G::string_view> word( line.substr(out.tailpos) , " \t"_sv ) ;
		for( ; word ; ++word )
		{
