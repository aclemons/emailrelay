...
std::unique_ptr<GSsl::LibraryImpBase> GSsl::Library::newLibraryImp( G::StringArray & library_config , Library::LogFn log_fn , bool verbose )
{
	...
	{
		return std::unique_ptr<LibraryImpBase>( new MbedTls::LibraryImp( library_config , log_fn , verbose ) ) ;
	}
	else
	{
		LibraryImpBase::consume( library_config , "openssl" ) ;
		return std::unique_ptr<LibraryImpBase>( new OpenSSL::LibraryImp( library_config , log_fn , verbose ) ) ;
	}
}
