...
std::string Main::Options::spec( bool is_windows )
{
	...
	std::string common(
		...
		"z!filter!specifies an external program to process messages as they are stored!!1!program!3|"
			...
			// daemon over the network, or "spam:<transport-address>" to talk to a
			// spamassassin spamd daemon, or "exit:<number>" to emulate a filter
		...
		"D!domain!sets an override for the host's fully qualified network name!!1!fqdn!3|"
			...
			// The default network name it derived from a DNS lookup of the local
			// hostname.
		...
		"o!forward-to!specifies the address of the remote SMTP server! (required by --forward, --forward-on-disconnect, --immediate and --admin)!1!host:port!3|"
			//example: smtp.example.com:25
		...
		"m!immediate!enables immediate forwarding of messages as soon as they are received! from the submitting client and before their receipt is acknowledged (requires --forward-to)!0!!3|"
			// Causes mail messages to be forwarded as soon as they are submitted,
			// even before they are accepted. This can be used to do proxying
		...
		"I!interface!defines the listening network addresses used for incoming connections! (comma-separated list with optional smtp=,pop=,admin= qualifiers)!2!ip-address-list!3|"
			//example: 192.168.0.1,127.0.0.1,::1
		...
		"Y!client-filter!specifies an external program to process messages when they are forwarded!!1!program!3|"
			...
			// are also allowed. The --filter option is normally more useful
			// than --client-filter.
		...
		"B!pop!enables the pop server!!0!!3|"
			...
			// access to spooled mail messages. Negotiated TLS using the POP3 "STLS"
		...
		"J!pop-by-name!modifies the pop spool directory according to the pop user name! (requires --pop)!0!!3|"
			...
			// without interfering with each other. Content files can stay in
			// the main spool directory with the envelope files copied into
			// user-specific sub-directories. The "emailrelay-filter-copy"
			// program is a convenient way of doing this when run via
			// --filter.
